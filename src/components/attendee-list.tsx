...
export function AttendeeList() {
  ...
  useEffect(() => {
    const url = new URL('http://localhost:3333/events/2103f765-851e-4c24-9db8-8a54e2300a76/attendees')
     ...
     <div className="flex gap-4 items-center">
      ...
      <Table>
          ...
          {attendees.map((attendee) => 
            (
              <TableRow key={attendee.id} className="border-b border-white/10 hover:bg-white/5">
                ...
                <TableCell>{attendee.checkedIn === null ? <span className="text-zinc-400">check-in not realized</span> : dayjs(attendee.checkedIn).fromNow()}</TableCell>
                <TableCell>
        ...
        <tfoot>
          <tr>
            <TableCell colSpan={3}>
              Show {attendees.length} of {total} items
            </TableCell>
